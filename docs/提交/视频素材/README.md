...
| swarm_breakdown.mp4（286.1 s，18 MB）/ swarm_breakdown_720p.mp4（11 MB） | 工作流拆解：球球怎么同时指挥二十条 Claude Code 线（总指挥 + 线 + 指挥板），token 消耗，对照 EvoMap / Hermes Agent。纸灰底数据动画（`scripts/video_tools/wf/`，13 个场景）+ 场景内字幕；旁白 MiniMax 预设男声 presenter_male（`vo/sw*.wav`）；配乐 Meditation Impromptu 02（钢琴，CC BY 4.0）。说明 / 数字口径 / 旁白全文见 [蜂群-拆解.md](../../蜂群-拆解.md)，重跑 `scripts/cut_swarm.sh`（数据集在 `workflow_data/`，不进 git） |
