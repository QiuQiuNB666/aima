...
| swarm_breakdown.mp4（281.8 s，34 MB）/ swarm_breakdown_720p.mp4（23 MB） | 蜂群拆解：我们的 Agent 蜂群 × 主办方 EvoMap × Hermes Agent。本机 `--sim` 仪表盘录屏（`frames/swarm_*`）+ 图卡 + 逐句字幕；旁白是 MiniMax 预设男声 presenter_male（`vo/sw*.wav`），峰哥复刻声只用在两句峰哥语录。说明 / 对比 / 旁白全文见 [蜂群-拆解.md](../../蜂群-拆解.md)，重跑 `scripts/cut_swarm.sh` |
