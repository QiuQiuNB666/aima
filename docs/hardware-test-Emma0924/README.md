...
2026-09-24。**可以测试网页演练、控制器模拟与历史遥测回放；真实手部行程标定未通过，尚不能启用实机自动抬杆、阻尼或后坐力。** 本轮已纳入同事 v0.8 更新，现场数据来源仍单独固定为 v0.6。

## 版本与合并

- 本交接分支：`codex/hardware-handoff-Emma0924`；现场测试来源为 `emma0924/upper-limb-improvements` 提交 `8f5a817dab9c576a5aa7046d0fa09888168f6812`（v0.6），首次回传快照为 `7fd667b`。
- 为解决文档冲突并便于同事直接测试，交接分支已纳入同事 `ec4dec3fcbb1b98b7f62f4a6b6b917459ae59987`（v0.8）的完整更新，保留其公共保护代码及教学。以独立增量 PR 供合并，不修改同事分支或 main；并未在 v0.8 上重新进行实机采集。
- 本次增量：修复 Windows 相同主机时刻下不同设备帧被误拒的问题；新增便携诊断/离线观察工具、测试、32 份去标识记录及交接说明。`shellos/hands/capture.py` 只放宽相同主机时刻，仍拒绝时间倒退并过滤重复设备帧。
- 推荐先将 PR #4 合入上肢开发分支，再按既有 PR #3 → PR #2 的顺序进入 Emma 分支和 main。无需复制整目录或重复挑选上肢代码。
...
打开 <http://127.0.0.1:4177/hands.html>，选择单套双手或双套的模拟演练模式，完成页面准备与教学后试玩靶场/挖掘机。该网页不自动打开串口或驱动电机。本地 4188 是现场端口设置，不是项目默认值；尚未部署常驻手部遥测桥，历史文件也没有伪装成实时设备数据。
...
最终交接已纳入 v0.8：Node **118/118** 通过；Python 相关回归及交接工具 **171 passed、2 deselected、2 subtests passed**。32 份记录完整性校验通过。复核范围和早期基线结果见本目录 `RELEASE-CHECKS.md`；这些是本机验证，不冒充 GitHub CI 或实机力反馈测试。
