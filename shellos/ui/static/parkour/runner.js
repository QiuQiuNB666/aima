...
export const RUN = {         // 现场调这里
  ...
  PP: v => clamp(22 + 3.6 * v, 30, 86),   // 目标髋角峰峰值（°）：7.5 m/s ≈ 49，10 ≈ 58，15 ≈ 76，17 ≈ 83（冲刺；9/24 上限 78 → 86）
  GMAX: 2.8,                 // 髋摆幅最多放大几倍（9/24 2.4 → 2.8：真人 150–180 步/分峰峰值 ~30° 也能放到冲刺幅度）
  HIP_FWD: 9,                // 跑步时髋整体往前偏（跑步髋角大约 −15…+45，均值偏屈）
  KNEE_ST: 24, KNEE_SW: 82,  // 支撑期缓冲屈膝 / 摆动期再屈多少（脚跟往屁股收）
  LEAN: v => clamp(3 + 0.9 * v, 4, 18),   // 躯干前倾（°）随速度（9/24 上限 15 → 18）
...
export function makeRunner(av) {
  ...
  function runLayer(P, dt, dd) {
    ...
    for (const [a, s] of [[P.armL, 1], [P.armR, -1]]) {
      a[0] *= 1 + RUN.ARM_GAIN * r * (1 + 0.35 * clamp((v - 9) / 8, 0, 1));   // 越快摆臂越猛
