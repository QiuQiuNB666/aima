...
function parts() {
  ...
  // 发动机罩：机舱顶上一条圆角的盒子
  body.push(loft([[0.55, 0.08, 0.06, 2.55], [0.4, 0.5, 0.28, 2.55], [-0.4, 0.56, 0.32, 2.56], [-1.4, 0.5, 0.28, 2.5], [-1.95, 0.25, 0.14, 2.35]], 12, 3, () => COWL));
  body.push(at(new THREE.CylinderGeometry(0.09, 0.09, 0.3, 8).rotateZ(Math.PI / 2), -2.05, 2.42, -0.28, '#55585e'));    // 排气管
  body.push(at(new THREE.CylinderGeometry(0.09, 0.11, 0.42, 8), 0, 2.88, 0, DARK));                                     // 旋翼轴
  // 尾梁（锥形）、水平尾翼 + 端板、上下垂尾（后掠）、尾撬
  body.push(at(new THREE.CylinderGeometry(0.13, 0.26, 4.9, 10).rotateZ(Math.PI / 2), -4.8, 1.9, 0, RED));
  body.push(at(new THREE.BoxGeometry(0.46, 0.06, 2.1), -6.05, 1.95, 0, RED));
  for (const z of [-1.05, 1.05]) body.push(at(new THREE.BoxGeometry(0.44, 0.5, 0.05), -6.1, 1.97, z, RED));
  ...
  body.push(tube(new THREE.Vector3(-7.45, 1.35, 0), new THREE.Vector3(-7.75, 1.2, 0), 0.03, DARK, 5));
  // 撬式起落架：两根滑橇（前头上翘）+ 两根弓形横管
  for (const z of [-1.15, 1.15]) {
    body.push(tube(new THREE.Vector3(-1.95, 0.05, z), new THREE.Vector3(1.65, 0.05, z), 0.055, DARK));
    body.push(tube(new THREE.Vector3(1.65, 0.05, z), new THREE.Vector3(2.05, 0.32, z), 0.055, DARK));
    for (const x of [0.95, -1.05]) {
      body.push(tube(new THREE.Vector3(x, 0.05, z), new THREE.Vector3(x, 0.45, z * 0.86), 0.05, DARK));
      body.push(tube(new THREE.Vector3(x, 0.45, z * 0.86), new THREE.Vector3(x, 0.62, z * 0.55), 0.05, DARK));
    }
  }
  for (const x of [0.95, -1.05]) body.push(tube(new THREE.Vector3(x, 0.62, -0.64), new THREE.Vector3(x, 0.62, 0.64), 0.05, DARK));
  body.push(at(new THREE.CylinderGeometry(0.1, 0.1, 0.16, 10).rotateZ(Math.PI / 2), 1.85, 0.62, 0, '#fff2c0'));        // 探照灯（机腹前）
  // 主旋翼：桨毂（星形）+ 3 片桨，每片从根到尖扭 8° → 1°、桨尖一段黄
  rotor.push(at(new THREE.CylinderGeometry(0.26, 0.26, 0.14, 12), HUB.x, HUB.y, HUB.z, '#55585e'));
  for (let k = 0; k < 3; k++) {
    const L = 5.1, g = new THREE.BoxGeometry(L, 0.045, 0.34, 8, 1, 1).toNonIndexed(), p = g.attributes.position, cols = [], cr = new THREE.Color(DARK), ct = new THREE.Color(TIP);
    ...
    rotor.push(at(new THREE.BoxGeometry(0.34, 0.08, 0.16).rotateY(k * Math.PI * 2 / 3), HUB.x + 0.2 * Math.cos(-k * Math.PI * 2 / 3), HUB.y, HUB.z + 0.2 * Math.sin(-k * Math.PI * 2 / 3), DARK));
  }
  // 尾桨：机身左侧，2 片，绕横轴（z）转
  tail.push(at(new THREE.BoxGeometry(0.15, 1.86, 0.035), TAIL.x, TAIL.y, TAIL.z, DARK));
  tail.push(at(new THREE.CylinderGeometry(0.07, 0.07, 0.1, 8).rotateX(Math.PI / 2), TAIL.x, TAIL.y, TAIL.z - 0.03, '#55585e'));
  // 「救援」：机舱后段下半（白）两侧
  ...
  for (const sd of [1, -1]) {
    const g = new THREE.PlaneGeometry(0.95, 0.36, 8, 2); if (sd < 0) g.rotateY(Math.PI);
...
export function buildHeliMesh() {
  ...
  const geo = mergeGeometries(groups).scale(S, S, S);
  ...
  const mesh = new THREE.Mesh(geo, spin(new THREE.MeshLambertMaterial({ vertexColors: true, map: decalTexture() })));
  mesh.customDepthMaterial = spin(new THREE.MeshDepthMaterial({ depthPacking: THREE.RGBADepthPacking }));
  mesh.name = 'heliBody';
  return { mesh, setSpin(rot, tail) { U.uRot.value = rot % (Math.PI * 2); U.uTail.value = tail % (Math.PI * 2); }, nose: new THREE.Vector3(1.85, 0.55, 0).multiplyScalar(S), rotorR: 5.34 * S, hubY: HUB.y * S };
}
