...
//   后左 → 前左 → 后右 → 前右，各差 1/4 拍）；毛色 / 驮包颜色也是实例属性（aFur / aPack），同一套几何每头看着不一样。
//   LOD 三级：0 ≈ 1k / 1 ≈ 450 / 2 ≈ 180 三角形 / 头（yakGeo(kit, lod)）。
...
export function yakGeo(kit, lod = 0) {
  const G = [], add = (g, part = 0, mask = 1) => G.push(mark(g, part, mask)), L1 = lod >= 1, L2 = lod >= 2;
  const ico = (sx, sy, sz, d = 1) => new THREE.IcosahedronGeometry(1, L1 ? 0 : d).scale(sx, sy, sz);
  // 身子（屁股 → 胸，肩高）+ 肩峰
  add(loft([[-1.08, 0.1, 0.12, 1.02], [-0.92, 0.33, 0.36, 1.0], [-0.45, 0.42, 0.42, 1.0], [0.1, 0.45, 0.45, 1.03], [0.45, 0.43, 0.5, 1.1], [0.72, 0.34, 0.42, 1.08], [0.92, 0.2, 0.26, 1.02]], L2 ? 6 : L1 ? 8 : 12, 2.3, () => FUR));
  add(at(ico(0.46, 0.24, 0.3), 0.38, 1.46, 0, FUR));
  // 长毛裙：椭圆筒从肚子垂到蹄子边，下沿一缕一缕参差
  {
    const g = new THREE.CylinderGeometry(0.46, 0.53, 0.74, L2 ? 8 : L1 ? 12 : 18, L2 ? 1 : 3, true).toNonIndexed(), p = g.attributes.position;
  ...
  [[1, 1, 1], [2, 1, -1], [3, -1, 1], [4, -1, -1]].forEach(([k, fx, sz]) => {
    ...
    add(at(new THREE.CylinderGeometry(0.1, 0.066, 0.88, L2 ? 4 : L1 ? 5 : 7), x, HIP[1] - 0.44 + 0.02, z, FUR_D), k, 1);
    if (!L2) add(at(new THREE.CylinderGeometry(0.07, 0.078, 0.08, L1 ? 5 : 7), x, 0.04, z, HOOF), k, 0);
  });
  ...
  if (!L1) add(at(ico(0.12, 0.09, 0.15, 0), 1.12, 1.06, 0, FUR_D), 5, 1);
  for (const sd of [1, -1]) {
    if (!L1) add(at(ico(0.08, 0.035, 0.11, 0), 1.02, 0.99, sd * 0.2, FUR), 5, 1);
    const P = [V(1.06, 1.04, sd * 0.12), V(1.04, 1.09, sd * 0.3), V(1.1, 1.25, sd * 0.4), V(1.21, 1.36, sd * 0.32)];
    if (L2) add(tube(P[0], P[3], 0.04, HORN, 3), 5, 0);
    else [[0, 0.048, HORN], [1, 0.036, HORN], [2, 0.022, HORN_T]].forEach(([i, r, c]) => add(tube(P[i], P[i + 1], r, c, L1 ? 4 : 6), 5, 0));
    if (!L1) add(at(new THREE.BoxGeometry(0.05, 0.12, 0.05), 1.08, 0.95, sd * 0.17, RED), 5, 0);
  }
  if (!L2) add(at(ico(0.07, 0.07, 0.07, 0), 1.2, 1.0, 0, RED), 5, 0);
  if (!L2) add(at(new THREE.ConeGeometry(0.065, 0.12, L1 ? 5 : 8), 0.98, 0.7, 0, GOLD), 5, 0);
  if (!L1) add(at(new THREE.BoxGeometry(0.05, 0.05, 0.3), 0.96, 0.8, 0, '#3a3226'), 5, 0);
  // 尾巴 + 尾穗
  add(tube(V(-1.02, 1.02, 0), V(-1.13, 0.64, 0), 0.035, FUR, L2 ? 3 : 5), 6, 1);
  if (!L2) add(at(ico(0.1, 0.24, 0.1), -1.15, 0.5, 0, FUR_D), 6, 1);
  // 驮包：两侧褡裢（白底 × aPack）+ 深色边、红盖布 + 黄边、顶上一捆绿帆布货、绳
  for (const sd of [1, -1]) {
    add(at(new THREE.BoxGeometry(0.6, 0.42, 0.18), 0.02, 1.06, sd * 0.49, '#f2eee6'), 0, 2);
    if (!L2) add(at(new THREE.BoxGeometry(0.62, 0.05, 0.19), 0.02, 1.2, sd * 0.49, '#9a968c'), 0, 2);
  }
  add(at(new THREE.BoxGeometry(0.78, 0.05, 1.08), 0.02, 1.49, 0, RED), 0, 0);
  if (!L1) for (const sd of [1, -1]) add(at(new THREE.BoxGeometry(0.8, 0.06, 0.05), 0.02, 1.49, sd * 0.54, '#e0a93a'), 0, 0);
  add(at(new THREE.CylinderGeometry(0.17, 0.17, 0.82, L2 ? 5 : L1 ? 7 : 10).rotateX(Math.PI / 2), 0.02, 1.66, 0, '#4f6b4a'), 0, 0);
  if (!L1) for (const x of [-0.18, 0.22]) add(at(new THREE.BoxGeometry(0.03, 0.03, 1.12), x, 1.8, 0, '#3a3226'), 0, 0);
