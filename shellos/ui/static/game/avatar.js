...
//   leg_joint_*_1 = 髋，_2 = 膝；髋绕局部 X 正转 = 大腿前抬（屈曲）；髋角约定 负 = 屈曲 → flex = -frame.l
// 影子登山者用同一个模型，换成半透明材质。
...
// 手臂（9/23 浏览器里试出来的）：肩绕局部 Z 负转 = 放下（-65° ≈ 自然下垂）；再绕局部 Y 正转 = 左臂前摆 / 右臂后摆
const ARM_DOWN = -65, ARM_SWING = 0.6;
let bufP = null;
...
export async function loadAvatar({ ghost = false, color = '#9fe8ff', opacity = 0.42 } = {}) {
  ...
  const arm = (b, swing) => { if (b) b.quaternion.copy(rest.get(b)).multiply(qA.setFromAxisAngle(Z, ARM_DOWN * d2r)).multiply(qT.setFromAxisAngle(Y, swing * d2r)); };
  const set = (b, deg) => { if (b) b.quaternion.copy(rest.get(b)).multiply(qT.setFromAxisAngle(X, deg * d2r)); };
