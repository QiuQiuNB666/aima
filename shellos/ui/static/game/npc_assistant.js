...
//   ?asst=<模型 id> 换人（models/assistant_q/ 里 8 个，占位 Vita），头发 / 裙摆用 VRM 自带的 spring bone。
...
export const VRM_ID = Q.get('asst') || 'Vita';   // 球球从候选总览图（docs/提交/截图/asst_q_candidates.png）里选定后改这里
...
function pointer(group, B, deg) {
  ...
  return w => {
    ...
    ax.set(0, 0, 1).applyQuaternion(group.getWorldQuaternion(qb));             // 人物的左右轴（世界）
...
function actor(group, B) {
  ...
  const rot = (b, x, y, z, deg) => {
    ...
    ax.set(x, y, z).applyQuaternion(group.getWorldQuaternion(qb)).applyQuaternion(b.getWorldQuaternion(qb).invert()).normalize();
