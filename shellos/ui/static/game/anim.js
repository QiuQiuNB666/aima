...
// slope 路面坡度（脚放平用）；arm 摆臂系数；elb 肘额外弯
export const KIND = {
  flat:        { lean: 3,  k0: 5,  kSw: 55, climb: 0.15, lift: 0,  slope: 0,  arm: 0.9, elb: 0 },
  up:          { lean: 10, k0: 9,  kSw: 60, climb: 0.4,  lift: 5,  slope: 7,  arm: 0.7, elb: 8 },
  stairs_up:   { lean: 12, k0: 10, kSw: 45, climb: 0.9,  lift: 18, slope: 0,  arm: 0.55, elb: 12 },
  down:        { lean: -5, k0: 11, kSw: 48, climb: 0.3,  lift: 0,  slope: -7, arm: 0.75, elb: 4 },
  stairs_down: { lean: -3, k0: 12, kSw: 42, climb: 0.45, lift: 4,  slope: 0,  arm: 0.6, elb: 8 },
  wait:        { lean: 1,  k0: 5,  kSw: 55, climb: 0.15, lift: 0,  slope: 0,  arm: 0.9, elb: 0 },
};
...
export function makeBody({ L1 = 0.26, L2 = 0.27, calib = true } = {}) {
  const K = { ...KIND.flat };
  const leg = () => ({ hi: 0, lo: 0, wpk: 60, init: false });
  const lg = [leg(), leg()];
  let off = 0, mean = null, act = 0, idle = 1, sm = 0, sway = 0, tIdle = 0;
  ...
  return {
    P,
    get act() { return act; }, get off() { return off; },
    // d = { fl, fr, wl, wr, kind, summit }
    update(dt, t, d) {
      dt = clamp(dt, 0, 0.1);
      ...
      const h = [d.fl - off, d.fr - off], w = [d.wl || 0, d.wr || 0];
      ...
      const sw = [0, 0];
      for (let k = 0; k < 2; k++) {
        const L = lg[k], wn = w[k] / L.wpk;
        sw[k] = clamp(wn * 1.4, 0, 1) * act;
        const hip = h[k] + K.lift * sw[k];
        // 蹬地：髋在后（低于包络中点）且刚开始前屈
        const push = clamp(wn * 3, 0, 1) * clamp(((L.hi + L.lo) / 2 - h[k]) / ((L.hi - L.lo) / 2 + 1), 0, 1) * act;
        const knee = Math.max(2, K.k0 + K.kSw * ampK * sw[k] + 25 * push + K.climb * Math.max(0, hip));
      ...
      const arm = (h[1] - h[0]) / 2 * K.arm * (1 - sm) + idle * 2 * Math.sin(t * 0.9);
      const down = 60 + 4 * idle + 1.5 * br * idle;
      const up = -85 + 14 * pump;                 // 登顶：双臂举成 V 字，一下一下挥
      const lc = 0.4 * lean * (1 - sm); P.armL[0] = arm - lc; P.armR[0] = arm + lc;   // 躯干前倾时手臂仍大致竖直垂着
      P.cheer = sm;
      P.armL[1] = P.armR[1] = down + (up - down) * sm;
      P.armL[2] = 4 + K.elb + Math.max(0, arm) * 0.5;   // 绑定姿态的肘本来就弯着一点
      P.armR[2] = 4 + K.elb + Math.max(0, -arm) * 0.5;
      ...
      P.bob = Math.max(vert(P.hipL, P.kneeL), vert(P.hipR, P.kneeR)) - (L1 + L2) + 0.012 * (Math.max(sw[0], sw[1]) - 0.5 * act) + 0.004 * br * idle;
      return P;
    },
  };
}
