...
RULES = {
    "phase": [
        (r"太早|早了", "t_ext", +5), (r"太晚|太迟|晚了|迟了", "t_ext", -5), (r"早", "t_ext", -5), (r"晚|迟", "t_ext", +5),
    ...
    "dofc": [
        (r"太早|早了", "delay_s", +0.03), (r"太晚|太迟|晚了|迟了", "delay_s", -0.03),
        (r"早", "delay_s", -0.03), (r"晚|迟", "delay_s", +0.03),
        (r"轻|小|弱|少", "gain", -0.03), (r"重|大|强|多|再来", "gain", +0.03),
        (r"反|顶|反了|相反", "gain", "flip"),
    ],
    "terrain": [
        # 否定和「更/再」排最前：「不太明显」「更明显一点」「没什么感觉」「不够」都是要更强
        (r"(没|不)(太|怎么|什么|够)?(感觉|明显|够|到|强|大|重|有力)|感觉不到|没.*力", "strength", +0.5),
        (r"(再|更|还).{0,3}(明显|强|大|重|有力)", "strength", +0.5),
        (r"(轻|小|弱)一?点", "strength", -0.5), (r"(重|大|强)一?点", "strength", +0.5),    # 「轻一点」是要求，不是描述
        (r"轻|小|弱|再来", "strength", +0.5), (r"陡|累|重|大|强|太猛|明显", "strength", -0.5),
    ] + [
        # 时机：抱怨（太早 / 晚了）和要求（早一点）方向相反；上坡 / 台阶 / 下坡三个脉冲一起挪
        rule for k in ("t_push", "t_step", "t_brake") for rule in (
            (r"太早|早了|提前了", k, +5), (r"太晚|太迟|晚了|迟了", k, -5), (r"早", k, -5), (r"晚|迟", k, +5))
    ],
    "constant": [
