#!/bin/bash
# 一键：本机大脑 + SSH 反向隧道到展位 MacBook（隧道断了 3 s 后自动重连）。Ctrl-C 两个一起退。
...
if [ -n "$MAC" ]; then
  while true; do
    echo "[tunnel] $(date +%T) 连 $MAC（远端 127.0.0.1:$PORT → 本机大脑）"
    ...
    ssh -N -o ExitOnForwardFailure=yes -o ServerAliveInterval=5 -o ServerAliveCountMax=2 \
        -o ConnectTimeout=5 -o BatchMode=yes -R "$PORT:127.0.0.1:$PORT" "$MAC"
