...
MM_TIMEOUT = 15.0     # 9/23 实测一句 4.2 / 7.8 / 10.2 s，8 s 太紧；ShellOS 等 25 s（shellos/agent/voice.py）
...
def minimax(text: str) -> bytes:
    ...
    }, ensure_ascii=False).encode(), timeout=MM_TIMEOUT)   # 加上退到 say 的时间，要在 ShellOS 那边 25 s 之内
