...
RUN=/tmp/booth-$PORT                 # $RUN.args = 上次的 ShellOS 参数（重起沿用）；$RUN.watchdog.pid / .log
...
case $MODE in
  watchdog)
    ...
    while :; do
      why=
      if ! pgrep -f "$PAT" >/dev/null; then why="进程没了"
  ...
  down)
    ...
    pkill -f "$PAT" 2>/dev/null; pkill -f "$CHROME_DIR" 2>/dev/null
  ...
  up)
    ...
    echo "--ctl terrain --stepping --cap $CAP --strength $STRENGTH --width 16 --http $PORT $SIM$EXTRA" > $RUN.args
    ./run-mac.sh $(cat $RUN.args)
    ...
    echo "== 3/5 看门狗 =="
    [ -f $RUN.watchdog.pid ] && kill "$(cat $RUN.watchdog.pid)" 2>/dev/null
    ...
    echo "== 5/5 绿灯清单 =="
    scripts/booth_check.sh --http "$PORT" ;;
esac
