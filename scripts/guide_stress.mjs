...
const OUT = flag('out', '');
const SCEN = MIX ? ['enter-listen', 'enter-walk', 'r2', 'reload', 'enter-walk', 'idle', 'estop', 'enter-walk'] : ['enter-walk'];
...
function judge(g, scen) {             // 这一轮哪里不对：返回失败类型列表（空 = 正常）
  ...
  if (log.some(x => /:nowav$|:NotSupportedError$/.test(x))) f.push('②缺 wav');
  if (log.some(x => /:stuck$/.test(x))) f.push('②卡住等兜底');
  if (overlaps(probe)) f.push('③叠着念');
  const played = new Set(guideSrc.filter(p => p[1] === 'playing').map(p => p[2]));
  if (says.length && played.size < says.length && !f.includes('①自动播放被拦')) f.push('①有句没声音');
  return { f, says: says.length, played: played.size };
}
